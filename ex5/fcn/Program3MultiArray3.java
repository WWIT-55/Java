...
public class Program3MultiArray3 {
	...
	static void printSmall(String[][] rows, int count) {
		{
...

			{
				...
				for (int i = 0; i < count; i++) {
					...
					if (old != current) {
						...
						diff = current - old;/*
												 * 여기도 이해안됌.커런트는 줄을의미ㅡ 올드는 현재 확진자수를의미 그러면 한줄에서 현재 확진자수를 빼주는의미인건가?
												 */
						// date에 rows[][]를 대입
						date = rows[i][0];
